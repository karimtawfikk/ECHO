...
export default function LoginPage() {
  ...
  return (
    <PageShell fullScreen minimal>
      ...
      `}</style>
      <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-[#0D0A07]">
        {/* Cinematic Background Elements */}
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
          <div className="absolute inset-0 bg-[url('/bg-pattern.png')] opacity-10 mix-blend-overlay" />
          
          {/* Ancient Embers (Glowy Spots) */}
          {mounted && [...Array(40)].map((_, i) => {
            ...
            return (
              <motion.div
                key={i}
                className="absolute rounded-full bg-[#E6B23C]"
                style={{
                  width: size,
                  height: size,
                  filter: `blur(${blur}px)`,
                  boxShadow: `0 0 ${glow}px #E6B23C`,
                }}
                initial={{
                  x: Math.random() * 100 + "%",
                  y: Math.random() * 100 + "%",
          ...
          {/* Very subtle corner lighting for depth only */}
          <div className="absolute top-[-10%] left-[-10%] w-[30%] h-[30%] bg-[#E6B23C]/5 rounded-full blur-[100px]" />
          <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-[#E6B23C]/5 rounded-full blur-[120px]" />
        </div>

        <div className="relative z-10 w-full min-h-screen flex flex-col md:flex-row">
          
          {/* Left Side: Immersive Image + Branding */}
          <div className="flex-1 relative overflow-hidden min-h-[500px] md:min-h-screen">
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              <img 
                ...
                alt="Ancient Egyptian Discovery" 
                className="absolute inset-0 w-full h-full object-cover"
              />
              {/* Left-to-Right Fade (For text readability) */}
              <div className="absolute inset-0 bg-gradient-to-r from-[#0D0A07]/90 via-[#0D0A07]/40 to-transparent" />
              
              {/* Right-Side Blend (To merge with background) */}
              <div className="absolute inset-y-0 right-0 w-48 bg-gradient-to-l from-[#0D0A07] to-transparent z-10" />
            </motion.div>

            <div className="relative h-full flex flex-col justify-center px-8 md:px-20 lg:px-32 py-20 z-10">
              <motion.div
              ...
              >
                <h1 className="text-6xl md:text-8xl font-heading font-bold text-white leading-[1.1] mb-8">
                  Unveil the <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E6B23C] to-[#B48B2D]">
                    Past.
                  </span>
                </h1>
                <p className="text-[#F5E6D0]/80 text-xl md:text-2xl leading-relaxed font-light">
                  Step into the digital archives of Ancient Egypt. Every capture has an origin, and every discovery begins here.
                </p>

                <div className="mt-12 flex items-center gap-6">
                  <div className="w-12 h-[1px] bg-[#E6B23C]/50" />
                  <span className="text-[10px] uppercase tracking-[0.5em] text-[#E6B23C] font-bold">
                    ECHO Archive System v1.0
                  </span>
                </div>
              </motion.div>
            </div>
          ...
          <div className="w-full md:w-[500px] lg:w-[600px] flex items-center justify-center p-8 bg-[#0D0A07] relative z-20">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="w-full max-w-[440px]"
            >
              <div className="relative p-1 rounded-[32px] bg-gradient-to-b from-[#E6B23C]/20 to-transparent backdrop-blur-xl">
                <div className="bg-[#0D0A07]/90 backdrop-blur-3xl rounded-[30px] p-8 md:p-10 border border-white/5 shadow-2xl">
                  ...
                  <div className="space-y-6">
                    <form onSubmit={handleEmailAuth} className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          <Mail className="absolute left-4 top-3.5 h-4 w-4 text-[#E6B23C]/40" />
                          <Input
                            id="email"
                            type="email"
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          <Lock className="absolute left-4 top-3.5 h-4 w-4 text-[#E6B23C]/40" />
                          <Input
                            id="password"
                          ...
                          <button
                            ...
                            className="absolute right-4 top-3 text-[#A08E70]/40 hover:text-[#E6B23C] transition-colors"
                          >
                      ...
                      <Button
                        type="submit"
                        disabled={loading}
                        className="w-full h-12 mt-4 rounded-xl bg-transparent border-2 border-[#E6B23C] text-[#E6B23C] font-bold uppercase tracking-widest hover:bg-[#E6B23C]/10 hover:shadow-[0_0_30px_rgba(230,178,60,0.2)] transition-all flex items-center justify-center gap-2"
                      >
                        {loading ? (
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="w-full h-11 rounded-xl bg-white/5 border-white/10 text-white hover:bg-white/10 hover:border-[#E6B23C]/30 transition-all flex items-center justify-center gap-3 text-xs group"
                    >
                    ...
                    <div className="text-center pt-2">
                      <div className="text-[#A08E70] text-sm flex items-center justify-center gap-2">
                        <span>{isSignUp ? "Already a member?" : "New explorer?"}</span>
                        <button
                          type="button"
